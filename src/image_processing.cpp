...
{
    if (!grabResult) {
        if (errorMsg) *errorMsg = "[ImageProcessing] Grab result is null!";
        return false;
    }
    ...
    if (!pImageBuffer) {
        if (errorMsg) *errorMsg = "[ImageProcessing] Image buffer is null!";
        return false;
    }
    if (width <= 0 || height <= 0) {
        if (errorMsg) *errorMsg = "[ImageProcessing] Invalid image dimensions! Width: " + std::to_string(width) + ", Height: " + std::to_string(height);
        return false;
    }
    cv::Mat bayerFrame(height, width, CV_8UC1, (void*)pImageBuffer);
    if (bayerFrame.empty()) {
        if (errorMsg) *errorMsg = "[ImageProcessing] Retrieved empty bayer frame!";
        return false;
    }
    try {
        cv::cvtColor(bayerFrame, rgbFrame, cv::COLOR_BayerBG2RGB);
        if (rgbFrame.empty()) {
            if (errorMsg) *errorMsg = "[ImageProcessing] Bayer-to-RGB conversion produced empty frame!";
            return false;
        }
        return true;
    } catch (const cv::Exception& e) {
        if (errorMsg) *errorMsg = std::string("[ImageProcessing] OpenCV exception during Bayer-to-RGB conversion: ") + e.what();
        return false;
    } catch (const std::exception& e) {
        if (errorMsg) *errorMsg = std::string("[ImageProcessing] Standard exception during Bayer-to-RGB conversion: ") + e.what();
        return false;
    } catch (...) {
        if (errorMsg) *errorMsg = "[ImageProcessing] Unknown error during Bayer-to-RGB conversion.";
        return false;
    }
}
...
bool ImageProcessing::TryProcessImage(const Pylon::CGrabResultPtr& grabResult, double& targetX, double& targetY, std::string* errorMsg)
{
    ...
    contours.erase(std::remove_if(contours.begin(), contours.end(), [](const std::vector<cv::Point>& c) { return cv::contourArea(c) < 700.0; }), contours.end());
    ...
    if (errorMsg) *errorMsg = "[ImageProcessing] No valid target found.";
    return false;
}
